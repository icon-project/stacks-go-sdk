...
func (t *SmartContractTransaction) Clone() StacksTransaction {
	clone := *t
	return &clone
}
...
func (t *SmartContractTransaction) Serialize() ([]byte, error) {
	...
	payloadBytes, err := t.Payload.Serialize()
	if err != nil {
		return nil, fmt.Errorf("failed to serialize smart contract payload: %w", err)
	}
	buf = append(buf, payloadBytes...)
...
func (t *SmartContractTransaction) Deserialize(data []byte) error {
	...
	_, err = t.Payload.Deserialize(data[offset:])
	if err != nil {
		return fmt.Errorf("failed to deserialize smart contract payload: %w", err)
	}
...
func DeserializeTransaction(data []byte) (StacksTransaction, error) {
	...
	case stacks.PayloadTypeSmartContract, stacks.PayloadTypeVersionedSmartContract:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to deserialize smart contract payload: %w", err)
		}
		tx = contractTx
