...
func (o *GetBlocks200Response) UnmarshalJSON(data []byte) (err error) {
    // Define which properties are truly required
    ...
    // Define which properties are optional (can be omitted if empty)
    ...
    for _, optionalProperty := range optionalProperties {
        if _, exists := allProperties[optionalProperty]; !exists {
            // If an optional property doesn't exist, set it to null
            allProperties[optionalProperty] = json.RawMessage("null")
        }
    }

    // Reconstruct the JSON with all properties (required and optional)
    completeJSON, err := json.Marshal(allProperties)
    if err != nil {
        return err
    }

    // Unmarshal into the actual struct
    var varGetBlocks200Response _GetBlocks200Response
    err = json.Unmarshal(completeJSON, &varGetBlocks200Response)
    if err != nil {
        return err
    }
