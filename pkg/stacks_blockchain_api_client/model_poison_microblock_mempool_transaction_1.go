/*
Stacks Blockchain API

Welcome to the API reference overview for the [Stacks Blockchain API](https://docs.hiro.so/stacks-blockchain-api).        [Download Postman collection](https://hirosystems.github.io/stacks-blockchain-api/collection.json)

API version: v8.1.2
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package stacks_blockchain_api_client

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the PoisonMicroblockMempoolTransaction1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PoisonMicroblockMempoolTransaction1{}

// PoisonMicroblockMempoolTransaction1 struct for PoisonMicroblockMempoolTransaction1
type PoisonMicroblockMempoolTransaction1 struct {
	// Transaction ID
	TxId string `json:"tx_id"`
	// Used for ordering the transactions originating from and paying from an account. The nonce ensures that a transaction is processed at most once. The nonce counts the number of times an account's owner(s) have authorized a transaction. The first transaction from an account will have a nonce value equal to 0, the second will have a nonce value equal to 1, and so on.
	Nonce int32 `json:"nonce"`
	// Transaction fee as Integer string (64-bit unsigned integer).
	FeeRate string `json:"fee_rate"`
	// Address of the transaction initiator
	SenderAddress string `json:"sender_address"`
	SponsorNonce *int32 `json:"sponsor_nonce,omitempty"`
	// Denotes whether the originating account is the same as the paying account
	Sponsored bool `json:"sponsored"`
	SponsorAddress *string `json:"sponsor_address,omitempty"`
	PostConditionMode TokenTransferTransactionPostConditionMode `json:"post_condition_mode"`
	PostConditions []TokenTransferTransactionPostConditionsInner `json:"post_conditions"`
	AnchorMode TokenTransferTransactionAnchorMode `json:"anchor_mode"`
	TxStatus TokenTransferMempoolTransaction1TxStatus `json:"tx_status"`
	// A unix timestamp (in seconds) indicating when the transaction broadcast was received by the node.
	ReceiptTime int32 `json:"receipt_time"`
	// An ISO 8601 (YYYY-MM-DDTHH:mm:ss.sssZ) timestamp indicating when the transaction broadcast was received by the node.
	ReceiptTimeIso string `json:"receipt_time_iso"`
	TxType string `json:"tx_type"`
	PoisonMicroblock PoisonMicroblockTransactionPoisonMicroblock `json:"poison_microblock"`
}

type _PoisonMicroblockMempoolTransaction1 PoisonMicroblockMempoolTransaction1

// NewPoisonMicroblockMempoolTransaction1 instantiates a new PoisonMicroblockMempoolTransaction1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPoisonMicroblockMempoolTransaction1(txId string, nonce int32, feeRate string, senderAddress string, sponsored bool, postConditionMode TokenTransferTransactionPostConditionMode, postConditions []TokenTransferTransactionPostConditionsInner, anchorMode TokenTransferTransactionAnchorMode, txStatus TokenTransferMempoolTransaction1TxStatus, receiptTime int32, receiptTimeIso string, txType string, poisonMicroblock PoisonMicroblockTransactionPoisonMicroblock) *PoisonMicroblockMempoolTransaction1 {
	this := PoisonMicroblockMempoolTransaction1{}
	this.TxId = txId
	this.Nonce = nonce
	this.FeeRate = feeRate
	this.SenderAddress = senderAddress
	this.Sponsored = sponsored
	this.PostConditionMode = postConditionMode
	this.PostConditions = postConditions
	this.AnchorMode = anchorMode
	this.TxStatus = txStatus
	this.ReceiptTime = receiptTime
	this.ReceiptTimeIso = receiptTimeIso
	this.TxType = txType
	this.PoisonMicroblock = poisonMicroblock
	return &this
}

// NewPoisonMicroblockMempoolTransaction1WithDefaults instantiates a new PoisonMicroblockMempoolTransaction1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPoisonMicroblockMempoolTransaction1WithDefaults() *PoisonMicroblockMempoolTransaction1 {
	this := PoisonMicroblockMempoolTransaction1{}
	return &this
}

// GetTxId returns the TxId field value
func (o *PoisonMicroblockMempoolTransaction1) GetTxId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TxId
}

// GetTxIdOk returns a tuple with the TxId field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetTxIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TxId, true
}

// SetTxId sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetTxId(v string) {
	o.TxId = v
}

// GetNonce returns the Nonce field value
func (o *PoisonMicroblockMempoolTransaction1) GetNonce() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Nonce
}

// GetNonceOk returns a tuple with the Nonce field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetNonceOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Nonce, true
}

// SetNonce sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetNonce(v int32) {
	o.Nonce = v
}

// GetFeeRate returns the FeeRate field value
func (o *PoisonMicroblockMempoolTransaction1) GetFeeRate() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.FeeRate
}

// GetFeeRateOk returns a tuple with the FeeRate field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetFeeRateOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.FeeRate, true
}

// SetFeeRate sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetFeeRate(v string) {
	o.FeeRate = v
}

// GetSenderAddress returns the SenderAddress field value
func (o *PoisonMicroblockMempoolTransaction1) GetSenderAddress() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.SenderAddress
}

// GetSenderAddressOk returns a tuple with the SenderAddress field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetSenderAddressOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SenderAddress, true
}

// SetSenderAddress sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetSenderAddress(v string) {
	o.SenderAddress = v
}

// GetSponsorNonce returns the SponsorNonce field value if set, zero value otherwise.
func (o *PoisonMicroblockMempoolTransaction1) GetSponsorNonce() int32 {
	if o == nil || IsNil(o.SponsorNonce) {
		var ret int32
		return ret
	}
	return *o.SponsorNonce
}

// GetSponsorNonceOk returns a tuple with the SponsorNonce field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetSponsorNonceOk() (*int32, bool) {
	if o == nil || IsNil(o.SponsorNonce) {
		return nil, false
	}
	return o.SponsorNonce, true
}

// HasSponsorNonce returns a boolean if a field has been set.
func (o *PoisonMicroblockMempoolTransaction1) HasSponsorNonce() bool {
	if o != nil && !IsNil(o.SponsorNonce) {
		return true
	}

	return false
}

// SetSponsorNonce gets a reference to the given int32 and assigns it to the SponsorNonce field.
func (o *PoisonMicroblockMempoolTransaction1) SetSponsorNonce(v int32) {
	o.SponsorNonce = &v
}

// GetSponsored returns the Sponsored field value
func (o *PoisonMicroblockMempoolTransaction1) GetSponsored() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.Sponsored
}

// GetSponsoredOk returns a tuple with the Sponsored field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetSponsoredOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Sponsored, true
}

// SetSponsored sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetSponsored(v bool) {
	o.Sponsored = v
}

// GetSponsorAddress returns the SponsorAddress field value if set, zero value otherwise.
func (o *PoisonMicroblockMempoolTransaction1) GetSponsorAddress() string {
	if o == nil || IsNil(o.SponsorAddress) {
		var ret string
		return ret
	}
	return *o.SponsorAddress
}

// GetSponsorAddressOk returns a tuple with the SponsorAddress field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetSponsorAddressOk() (*string, bool) {
	if o == nil || IsNil(o.SponsorAddress) {
		return nil, false
	}
	return o.SponsorAddress, true
}

// HasSponsorAddress returns a boolean if a field has been set.
func (o *PoisonMicroblockMempoolTransaction1) HasSponsorAddress() bool {
	if o != nil && !IsNil(o.SponsorAddress) {
		return true
	}

	return false
}

// SetSponsorAddress gets a reference to the given string and assigns it to the SponsorAddress field.
func (o *PoisonMicroblockMempoolTransaction1) SetSponsorAddress(v string) {
	o.SponsorAddress = &v
}

// GetPostConditionMode returns the PostConditionMode field value
func (o *PoisonMicroblockMempoolTransaction1) GetPostConditionMode() TokenTransferTransactionPostConditionMode {
	if o == nil {
		var ret TokenTransferTransactionPostConditionMode
		return ret
	}

	return o.PostConditionMode
}

// GetPostConditionModeOk returns a tuple with the PostConditionMode field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetPostConditionModeOk() (*TokenTransferTransactionPostConditionMode, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PostConditionMode, true
}

// SetPostConditionMode sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetPostConditionMode(v TokenTransferTransactionPostConditionMode) {
	o.PostConditionMode = v
}

// GetPostConditions returns the PostConditions field value
func (o *PoisonMicroblockMempoolTransaction1) GetPostConditions() []TokenTransferTransactionPostConditionsInner {
	if o == nil {
		var ret []TokenTransferTransactionPostConditionsInner
		return ret
	}

	return o.PostConditions
}

// GetPostConditionsOk returns a tuple with the PostConditions field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetPostConditionsOk() ([]TokenTransferTransactionPostConditionsInner, bool) {
	if o == nil {
		return nil, false
	}
	return o.PostConditions, true
}

// SetPostConditions sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetPostConditions(v []TokenTransferTransactionPostConditionsInner) {
	o.PostConditions = v
}

// GetAnchorMode returns the AnchorMode field value
func (o *PoisonMicroblockMempoolTransaction1) GetAnchorMode() TokenTransferTransactionAnchorMode {
	if o == nil {
		var ret TokenTransferTransactionAnchorMode
		return ret
	}

	return o.AnchorMode
}

// GetAnchorModeOk returns a tuple with the AnchorMode field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetAnchorModeOk() (*TokenTransferTransactionAnchorMode, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AnchorMode, true
}

// SetAnchorMode sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetAnchorMode(v TokenTransferTransactionAnchorMode) {
	o.AnchorMode = v
}

// GetTxStatus returns the TxStatus field value
func (o *PoisonMicroblockMempoolTransaction1) GetTxStatus() TokenTransferMempoolTransaction1TxStatus {
	if o == nil {
		var ret TokenTransferMempoolTransaction1TxStatus
		return ret
	}

	return o.TxStatus
}

// GetTxStatusOk returns a tuple with the TxStatus field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetTxStatusOk() (*TokenTransferMempoolTransaction1TxStatus, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TxStatus, true
}

// SetTxStatus sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetTxStatus(v TokenTransferMempoolTransaction1TxStatus) {
	o.TxStatus = v
}

// GetReceiptTime returns the ReceiptTime field value
func (o *PoisonMicroblockMempoolTransaction1) GetReceiptTime() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.ReceiptTime
}

// GetReceiptTimeOk returns a tuple with the ReceiptTime field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetReceiptTimeOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ReceiptTime, true
}

// SetReceiptTime sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetReceiptTime(v int32) {
	o.ReceiptTime = v
}

// GetReceiptTimeIso returns the ReceiptTimeIso field value
func (o *PoisonMicroblockMempoolTransaction1) GetReceiptTimeIso() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ReceiptTimeIso
}

// GetReceiptTimeIsoOk returns a tuple with the ReceiptTimeIso field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetReceiptTimeIsoOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ReceiptTimeIso, true
}

// SetReceiptTimeIso sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetReceiptTimeIso(v string) {
	o.ReceiptTimeIso = v
}

// GetTxType returns the TxType field value
func (o *PoisonMicroblockMempoolTransaction1) GetTxType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TxType
}

// GetTxTypeOk returns a tuple with the TxType field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetTxTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TxType, true
}

// SetTxType sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetTxType(v string) {
	o.TxType = v
}

// GetPoisonMicroblock returns the PoisonMicroblock field value
func (o *PoisonMicroblockMempoolTransaction1) GetPoisonMicroblock() PoisonMicroblockTransactionPoisonMicroblock {
	if o == nil {
		var ret PoisonMicroblockTransactionPoisonMicroblock
		return ret
	}

	return o.PoisonMicroblock
}

// GetPoisonMicroblockOk returns a tuple with the PoisonMicroblock field value
// and a boolean to check if the value has been set.
func (o *PoisonMicroblockMempoolTransaction1) GetPoisonMicroblockOk() (*PoisonMicroblockTransactionPoisonMicroblock, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PoisonMicroblock, true
}

// SetPoisonMicroblock sets field value
func (o *PoisonMicroblockMempoolTransaction1) SetPoisonMicroblock(v PoisonMicroblockTransactionPoisonMicroblock) {
	o.PoisonMicroblock = v
}

func (o PoisonMicroblockMempoolTransaction1) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PoisonMicroblockMempoolTransaction1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["tx_id"] = o.TxId
	toSerialize["nonce"] = o.Nonce
	toSerialize["fee_rate"] = o.FeeRate
	toSerialize["sender_address"] = o.SenderAddress
	if !IsNil(o.SponsorNonce) {
		toSerialize["sponsor_nonce"] = o.SponsorNonce
	}
	toSerialize["sponsored"] = o.Sponsored
	if !IsNil(o.SponsorAddress) {
		toSerialize["sponsor_address"] = o.SponsorAddress
	}
	toSerialize["post_condition_mode"] = o.PostConditionMode
	toSerialize["post_conditions"] = o.PostConditions
	toSerialize["anchor_mode"] = o.AnchorMode
	toSerialize["tx_status"] = o.TxStatus
	toSerialize["receipt_time"] = o.ReceiptTime
	toSerialize["receipt_time_iso"] = o.ReceiptTimeIso
	toSerialize["tx_type"] = o.TxType
	toSerialize["poison_microblock"] = o.PoisonMicroblock
	return toSerialize, nil
}

func (o *PoisonMicroblockMempoolTransaction1) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"tx_id",
		"nonce",
		"fee_rate",
		"sender_address",
		"sponsored",
		"post_condition_mode",
		"post_conditions",
		"anchor_mode",
		"tx_status",
		"receipt_time",
		"receipt_time_iso",
		"tx_type",
		"poison_microblock",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varPoisonMicroblockMempoolTransaction1 := _PoisonMicroblockMempoolTransaction1{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varPoisonMicroblockMempoolTransaction1)

	if err != nil {
		return err
	}

	*o = PoisonMicroblockMempoolTransaction1(varPoisonMicroblockMempoolTransaction1)

	return err
}

type NullablePoisonMicroblockMempoolTransaction1 struct {
	value *PoisonMicroblockMempoolTransaction1
	isSet bool
}

func (v NullablePoisonMicroblockMempoolTransaction1) Get() *PoisonMicroblockMempoolTransaction1 {
	return v.value
}

func (v *NullablePoisonMicroblockMempoolTransaction1) Set(val *PoisonMicroblockMempoolTransaction1) {
	v.value = val
	v.isSet = true
}

func (v NullablePoisonMicroblockMempoolTransaction1) IsSet() bool {
	return v.isSet
}

func (v *NullablePoisonMicroblockMempoolTransaction1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePoisonMicroblockMempoolTransaction1(val *PoisonMicroblockMempoolTransaction1) *NullablePoisonMicroblockMempoolTransaction1 {
	return &NullablePoisonMicroblockMempoolTransaction1{value: val, isSet: true}
}

func (v NullablePoisonMicroblockMempoolTransaction1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePoisonMicroblockMempoolTransaction1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


